          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              >
                ...
                {cats
                  .filter(c => !c.isArchived && c.gender !== 'male' && c.id !== formData.fatherId && c.id !== (formData as any).id)
            ...
            <div>
              ...
              >
                <option value="">Дикая природа</option>
                {cats
                  .filter(c => !c.isArchived && c.gender !== 'female' && c.id !== formData.motherId && c.id !== (formData as any).id)
                  .map(c => <option key={c.id} value={c.id}>{c.name} {c.tags.includes('Племя') ? '🧬' : ''}</option>)}
              </select>
            </div>
          </div>
