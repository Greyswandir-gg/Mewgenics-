...
const TeamBuilderView: React.FC = () => {
  ...
  const handleAiAdvice = async () => {
    ...
    setLoadingAi(true);
    const simplifiedData = teamMembers.map(m => {
      const cat = cats.find(c => c.id === m.catId);
      return {
        name: cat?.name,
        stats: calculateCatStats(cat!, m.sandboxCollarId).stats
      };
    });
    const advice = await getTeamStrategy(simplifiedData);
    setAiAdvice(advice);
    setLoadingAi(false);
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-20">
        ...
        <div className="bg-[#fff9f0] border-4 border-black sketch-border p-10 relative overflow-hidden shadow-xl">
          <div className="relative z-10">
            <h2 className="text-3xl font-black uppercase italic mb-8 border-b-4 border-black pb-3 flex items-center gap-3">
              ✨ ТАКТИКА (ИИ)
            </h2>
            {aiAdvice ? (
              <div className="bg-white/60 p-6 border-4 border-black sketch-border-sm">
                <p className="text-black font-black text-lg leading-tight italic">{aiAdvice}</p>
              </div>
            ) : (
              <p className="text-black font-black italic text-xl opacity-30 text-center py-10">Соберите команду для анализа...</p>
            )}
            <button 
              onClick={handleAiAdvice}
              disabled={loadingAi || teamMembers.length === 0}
              className={`mt-10 w-full py-5 border-4 border-black font-black uppercase transition sketch-border shadow-none text-xl italic tracking-widest ${loadingAi || teamMembers.length === 0 ? 'bg-zinc-200 opacity-30 grayscale' : 'bg-black text-white hover:bg-zinc-800'}`}
            >
              {loadingAi ? 'АНАЛИЗИРУЮ...' : 'РАССЧИТАТЬ СТРАТЕГИЮ'}
            </button>
          </div>
          <div className="absolute -right-10 -bottom-10 opacity-5 text-[150px] pointer-events-none font-black italic rotate-12">GENAI</div>
        </div>
      </div>
    </div>
  );
